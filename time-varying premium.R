...
lm_1 <- lmList(ret_e ~ srjv | ym , data=da_m)
da_premium_1 <- data.table(ym=ymd(names(summary(lm_1)$coefficients[,,2][,1])),
                           premium_1=as.numeric(summary(lm_1)$coefficients[,,2][,1]))
da_premium_1[,premium_1:=-premium_1]

da_premium_1 <- merge(da_premium_1,da_business,by="ym")
#sd(da_premium_1[,business]) #3.442428

model_nw_1 <- lm(premium_1 ~ business,data=da_premium_1)
summary(model_nw_1)$coefficients
coeftest(model_nw_1,vcov = NeweyWest(model_nw_1))[2,3]

#### FF3F is controlled
da_m <- NULL
da_m <- merge(da_all_m,da_realized_m,by=c("ym","SecCode"))
da_m <- merge(da_m,da_beta_5y,by=c("ym","SecCode")) # +be
da_m <- merge(da_m,da_mom_m,by=c("ym","SecCode")) # +mom

lm_2 <- lmList(ret_e ~ srjv+be+log(size)+log(BM) | ym , data=da_m)
da_premium_2 <- data.table(ym=ymd(names(summary(lm_2)$coefficients[,,2][,1])),
                           premium_2=as.numeric(summary(lm_2)$coefficients[,,2][,1]))
da_premium_2[,premium_2:=-premium_2]

da_premium_2 <- merge(da_premium_2,da_business,by="ym")
model_nw_2 <- lm(premium_2 ~ business,data=da_premium_2)
summary(model_nw_2)$coefficients
coeftest(model_nw_2,vcov = NeweyWest(model_nw_2))[2,3]

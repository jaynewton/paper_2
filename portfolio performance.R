...
ret_p[,`:=`(cum_srjv=cumsum(ret_lh),cum_mkt_e=cumsum(mkt_e),
            cum_smb=cumsum(smb),cum_hml=cumsum(hml))]
ret_p[,`:=`(exp_cum_srjv=exp(cum_srjv),exp_cum_mkt_e=exp(cum_mkt_e),
...
ret_plot <- melt(ret_p,id.vars="ym",
                 measure.vars=c("exp_cum_srjv","exp_cum_mkt_e",
                                "exp_cum_smb","exp_cum_hml"),
...
#### cumulated return
ret_plot <- melt(ret_p,id.vars="ym",
                 measure.vars=c("cum_srjv","cum_mkt_e","cum_smb","cum_hml"),
...
ret_p <- as.data.table(na.omit(ret_p))
ret_p$ret_l <- ret_p[,1] # ret_p_l denotes low portfolio returns
ret_p[,ym:=sort(unique(da_m$ym))]
ret_p[,ym:=ym+months(1)] # the right month strategies are applied
...
ret_p <- merge(ret_p,FF3F_m,by="ym")
ret_p[,c(paste0("p",1:5)):=NULL]
ret_p <- ret_p[,.(ym,ret_l,mkt_e)]

ret_p[,`:=`(cum_srjv=cumsum(ret_l),cum_mkt_e=cumsum(mkt_e))]
ret_p[,`:=`(exp_cum_srjv=exp(cum_srjv),exp_cum_mkt_e=exp(cum_mkt_e))]
...
p+geom_line()+
  ...
  scale_linetype_discrete(guide=guide_legend(title=NULL),
                          labels=c("RSV","MKT"))
ggsave("C:/Users/Ding/Desktop/cumulated value_2.png",width=1.618*20,height=20,units="cm",dpi=500)

#### cumulated return
ret_plot <- melt(ret_p,id.vars="ym",
                 measure.vars=c("cum_srjv","cum_mkt_e"),
                 variable.name="strategies",value.name="cumulated_return")
...
p+geom_line()+
  ...
  scale_linetype_discrete(guide=guide_legend(title=NULL),
                          labels=c("RSV","MKT"))
ggsave("C:/Users/Ding/Desktop/cumulated return_2.png",width=1.618*20,height=20,units="cm",dpi=500)

#### Mean Return
ret_p[,sum(ret_l)/nrow(ret_p)*12]
ret_p[,sum(mkt_e)/nrow(ret_p)*12]

#### Value at Risk(VaR) 
ret_p[,quantile(ret_l,0.01)]
ret_p[,quantile(mkt_e,0.01)]
ret_p[,quantile(ret_l,0.05)]
ret_p[,quantile(mkt_e,0.05)]

#### Expected Shortfall
ret_p[ret_l<quantile(ret_l,0.01),mean(ret_l)]
ret_p[mkt_e<quantile(mkt_e,0.01),mean(mkt_e)]
ret_p[ret_l<quantile(ret_l,0.05),mean(ret_l)]
ret_p[mkt_e<quantile(mkt_e,0.05),mean(mkt_e)]

#### Sharpe Ratio (Yearly)
ret_p[,mean(ret_l)/sd(ret_l)]*sqrt(nrow(ret_p)/nrow(ret_p)*12) 
ret_p[,mean(mkt_e)/sd(mkt_e)]*sqrt(nrow(ret_p)/nrow(ret_p)*12)

#################################
#### Double Sort (Varible of Interest)
...
da_m <- merge(da_m,da_beta_5y,by=c("ym","SecCode")) # dataset 2

da_m <- merge(da_all_m,da_realized_m,by=c("ym","SecCode"))
da_m <- merge(da_m,da_mom_m,by=c("ym","SecCode")) # dataset 3
...
for (i in 1:length(ym_index)) {
  ...
  da_sub <- da_sub[order(rvol),] # dataset 1
  #da_sub <- da_sub[order(rsk),] # dataset 1
  #da_sub <- da_sub[order(be),] # dataset 2
  #da_sub <- da_sub[order(size),] # dataset 1
  #da_sub <- da_sub[order(BM),] # dataset 1
  #da_sub <- da_sub[order(mom),] # dataset 3
  #da_sub <- da_sub[order(ret_tm),] # dataset 1
  #da_sub <- da_sub[order(illiq),] # dataset 1
  ...
  for (j in 1:k) {
    ...
    da_ss <- da_ss[order(srjv),] # srjv is the variable of interest
